            ...
            for f_id in range(f_start_id + 1 , len(files)):
                ...
                for step, batch in enumerate(train_iter):
                    ...
                    if training_steps == 10:
                        ...
                        #_nvtx.nvtxRangePushA("Test")
                        CD.profiler.start()
                    elif training_steps == 20:
                        ##_nvtx.nvtxRangePop()
                        print("Ten-batch training time: ", alltime[19]-alltime[9])
                        CD.profiler.stop()
                        #CD.nvtx.RangePop()
                        print("cuda profile stop")
                    CD.nvtx.RangePush("Test: " + str(training_steps))
                    training_steps += 1
                    ...
                    if global_step >= args.steps_this_run or training_steps % (
                        ...
                        if global_step >= args.steps_this_run or timeout_sent:
                            ...
                            # thread.join()
                            #print("global_step: {0}, training_step: {1}, step: {2}, batch: {3}".format(global_step, training_steps, step, batch))
